...
class Dropout(Layer):
    ...
    def forward_propagation(self, input: np.ndarray, training: bool) -> np.ndarray:
        ...
        if training:
            ...
            self.scaling_factor = 1 / (1 - self.probability)
            # compute the mask using a binomial distribution with probability (1 - probability) and with the size equal to the input size
            self.mask = np.random.binomial(1, 1 - self.probability, size=input.shape)
            # compute the output (input * mask* scaling_factor)
    ...
    def backward_propagation(self, output_error: np.ndarray) -> float:
        """
        Perform backward propagation on the given output error.
        Multiply the output_error with the mask.
