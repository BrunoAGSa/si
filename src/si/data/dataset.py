...
class Dataset:
    ...
    def dropna(self) -> np.ndarray:
        ...
        nan = np.isnan(self.X).any(axis=1)
        self.X = self.X[~nan]
        self.y = self.y[~nan]
        return self

    
    def fillna(self, value: Union[int, float, str]) -> np.ndarray:

        """Replaces all null values (NaN) with the given value.
    ...
    def remove_by_index(self, index: int) -> np.ndarray:
